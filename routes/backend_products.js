...
function getBuilderImage(products) {
    products.forEach((product, idx, array) => {
        var prod_id = product.id;
        var file_url = './uploads/product_' + prod_id + '.png';
        if (fs.existsSync(file_url)) {
...
//get all shopify collections and their products
router.post('/get_products', function (req, res) {
    ...
    console.log('shop :', req.session);
    console.log('shopify_api_key :', apiKey);
    console.log('access_token :', req.session.access_token);

    var query = '';
    console.log('req.body.lastColId :', req.body.lastColId);
    if (req.body.lastColId == undefined) {
        query = '/admin/api/2020-04/smart_collections.json?limit=250';
    } else if (req.body.lastColId == 0) {
        query = '/admin/api/2020-04/smart_collections.json?limit=10';
    } else {
        query = '/admin/api/2020-04/smart_collections.json?limit=10&since_id=' + req.body.lastColId;
    }
    console.log('query :', query);
    // Shopify.get('/admin/api/2020-04/smart_collections.json?limit=250', function (err, collections, headers) {
    Shopify.get(query, function (err, collections, headers) {
        collections = collections.smart_collections;
        console.log('collections :', collections);
        if (collections.length > 0) {
            // collections.forEach((collection, idx, array) => {
...
router.post('/search', function (req, res) {
    ...
    var query = '/admin/api/2020-04/smart_collections.json?limit=250';
    Shopify.get(query, function (err, collections, headers) {
        var search_text = req.body.search_text;
        collections = collections.smart_collections;
        var filtered_collections = collections.filter(ele => {
            if (ele.title.toUpperCase().indexOf(search_text.toUpperCase()) > -1) {
                return ele;
            }
        });
        if (filtered_collections.length > 0) {
            function getProductInfo() {
                if (i <= filtered_collections.length - 1) {
                    var collection = filtered_collections[i];
                    Shopify.get('/admin/api/2020-04/products.json?collection_id=' + collection.id, function (err, products, headers) {
                        var params = {
                            collection: collection,
                            products: products
...
//Get selected collections and products
router.post('/get_selected', function (req, res) {
    var filter_products = [];
    var products_data = [];
    ...
    var query = '/admin/api/2020-04/smart_collections.json?limit=250';
    Shopify.get(query, function (err, collections, headers) {
        var selected_style_collections = req.body.selected_style_collections;
        collections = collections.smart_collections;
        var filtered_collections = [];
        if (selected_style_collections.top_collections != undefined) {
            filtered_collections = collections.filter(ele => {
                return selected_style_collections.top_collections.some((collection) => collection.collection_id == ele.id);
            });
        } else {
            filtered_collections = collections.filter(ele => {
                return selected_style_collections.bottom_collections.some((collection) => collection.collection_id == ele.id);
            });
        }
        if (filtered_collections.length > 0) {
            function getProductInfo() {
...
router.post('/edit_product', function (req, res) {
    ...
    upload(req, res, function (err) {
        ...
        if (req.file) {
            ...
            var path = './uploads/' + prod_image;
            var ext = prod_image.split('.')[1];
            var new_path = './uploads/product_' + product_id + '.' + ext;
